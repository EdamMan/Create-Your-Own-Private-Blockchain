...
class Blockchain {
    ...
    _addBlock(block) {
	  ...
	  try {
                ...
                block.time = this._gettingCurrentTime();
                if (newHeight > 0) 
		{
    ...
    requestMessageOwnershipVerification(address) {
        ...
        resolve(`${address}:${this._gettingCurrentTime()}:starRegistry`);    
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const currentTime = parseInt(this._gettingCurrentTime());
            const allowedDelay = 5 * 60;
            if (currentTime - time > allowedDelay) reject(new Error('Time period of ELAPSED is grater then ( > 5) minute'));
        ...
        return new Promise(async (resolve, reject) => {
            for (let i = 0; i < this.chain.length; i++) {
                ...
                if (currentBlock.previousBlockHash !== previousBlock.hash) {
                    errorLog.push({
                        error: 'last block was not matched',
                        block: currentBlock
                    });
                }
            }
